...
요청:
1. [시각화 해석] 구간에서 레이더 차트의 전체 분포, 가장 눈에 띄는 영역(가장 넓거나 좁은 부분, 교차·중첩 등)만 200자 이내로 간단히 요약.
2. 아래 예시처럼 심층 분석(조합별 강점·약점, 패턴, 전략적 시사점 등)을 소제목 구조로 500자 내외로 작성.
-은 모두 -로, 숫자 한 자리 소수, 조합명 반복, 소제목 포함.

[출력 예시]
---
[시각화 해석]
여성 | 30-34세의 정보 획득, 남성 | 40-44세의 문화-교육 영역이 두드러지며, 소통 및 정책 활용이 전체적으로 낮은 편이다. 일부 조합 간 만족도 프로파일의 교차가 뚜렷하다.
...
요청:
1. [시각화 해석] 구간에서 히트맵의 점수 분포, 가장 두드러진 고·저, 군집이나 색상 경계 등 200자 이내 요약.
2. 아래 예시처럼 그룹별 특징, 강점·약점, 전체 경향, 권장점 순으로 분석(500자 내외).
-은 모두 -로, 숫자 한 자리 소수, 조합명 반복, 소제목 포함.

[출력 예시]
---
[시각화 해석]
정보 획득과 문화-교육 향유 점수가 진하게 표시되어 높으며, 소통 및 정책 활용은 전체적으로 색이 옅다. 여성 조합끼리 점수가 몰려 뚜렷한 군집을 이룬다.
...
요청:
1. [시각화 해석] 구간에서 delta 그래프 상에서 양·음 방향으로 가장 큰 편차, 일관된 패턴, 특이 조합을 200자 이내로 요약.
2. 아래 예시처럼 패턴 분석·우선순위 권장점(500자 내외) 작성.
-은 모두 -로, 숫자 한 자리 소수, 조합명 반복, 소제목 포함.

[출력 예시]
---
[시각화 해석]
여성 | 30-34세의 정보 획득 편차(+11.3), 남성 | 40-44세의 문화-교육(+9.2)이 눈에 띄고, 다수 조합의 소통·정책 활용은 음의 편차다.
...
요청:
1. [시각화 해석]에서 신뢰구간·error bar 길이, 0선과의 관계, 주목할 조합을 200자 이내로 요약.
2. 아래 예시처럼 실질적 편차·불확실성·개입 우선순위 분석(500자 내외).
-은 모두 -로, 숫자 한 자리 소수, 조합명 반복, 소제목 포함.

[출력 예시]
---
[시각화 해석]
여성 | 30-34세, 남성 | 50-54세는 각각 error bar가 0선을 명확히 벗어나며, 여성 | 40-44세는 신뢰구간이 0을 포함한다.
...
요청:
1. [시각화 해석]에서 점수 폭, 최상·최하위 조합, 급격한 변화나 outlier 등 핵심 시각 특징을 200자 이내로 요약.
2. 아래 예시처럼 점수 분포·outlier·일관된 그룹 분석(500자 내외).
-은 모두 -로, 숫자 한 자리 소수, 조합명 반복, 소제목 포함.

[출력 예시]
---
[시각화 해석]
{midcat} 점수는 여성 | 30-34세, 여성 | 35-39세가 가장 높고, 남성 | 50-54세가 최하위다. 남성 | 40-44세만 매우 높은 outlier다.
