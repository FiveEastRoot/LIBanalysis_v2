...
요청:
1. 먼저 [시각화 해석] 구간에서 전체 레이더 차트에서 무엇이 가장 높고, 낮은지, 조합들 간에 눈에 띄는 간격이나 중복·교차되는 영역이 있는지 200자 내외로 설명.
2. 이어서 각 조합명 심층 분석과 인사이트를 다음 형식으로 작성.
(분석부턴 기존 예시와 동일하게, 소제목 포함, 500자 내외)

[시각화 해석 예시]
---
[시각화 해석]
전체적으로 정보 획득이 모든 조합에서 높은 편이며, 소통 및 정책 활용은 가장 낮게 나타난다. 여성 | 30-34세와 남성 | 40-44세의 만족도 패턴이 일부 교차하는 부분이 뚜렷하다.
...
요청:
1. 먼저 [시각화 해석] 구간에서 히트맵에서 높은 점수/낮은 점수, 뚜렷한 군집, 색상의 연속성이나 경계 등 눈에 띄는 시각적 특징을 200자 이내로 요약.
2. 이어서 조합별, 그룹별 분석과 권장점은 아래 예시처럼 작성.

[시각화 해석 예시]
---
[시각화 해석]
히트맵에서는 정보 획득, 문화-교육 향유 점수가 전체적으로 높고, 소통 및 정책 활용은 모든 조합에서 색이 옅게 분포한다. 여성 조합끼리 강한 군집이 보인다.

### 1. 그룹별 특징
여성 | 30-34세, 여성 | 35-39세는 정보 획득과 문화-교육 향유가 모두 높다. 남성 | 50-54세 등은 대체로 모든 영역에서 낮다.

### 2. 강점·약점 요약
여성 | 30-34세는 정보 획득, 남성 | 40-44세는 문화-교육이 강점이고, 다수 조합에서 소통 및 정책 활용이 약점이다.

### 3. 전체 경향 및 예외
전반적으로 정보 획득 점수가 높고, 소통은 모든 조합에서 낮은 경향이다. 여성 | 40-44세만 소통 점수가 평균보다 높다.
...
요청:
1. 먼저 [시각화 해석]에서 delta 차트의 가장 강하게 벗어난 값, 일관된 플러스/마이너스, 예외적으로 튀는 조합명을 200자 이내로 요약.
2. 이어서 편차 패턴 심층 분석과 개선 우선순위는 예시처럼 작성.

[시각화 해석 예시]
---
[시각화 해석]
여성 | 30-34세의 정보 획득은 다른 조합 대비 가장 큰 양의 편차를 보이며, 소통 및 정책 활용에서 음의 편차가 두드러진다. 40대 남성 조합은 문화-교육에서 높은 플러스, 정보 획득은 마이너스다.

### 1. 주요 편차 영역
여성 | 30-34세는 정보 획득에서 +11.3, 소통-정책 활용에서 -8.5로 강한 편차. 남성 | 40-44세는 문화-교육 향유에서 +9.2, 정보 획득에서 -6.1.

### 2. 그룹화 패턴
30-40대 여성 조합들이 정보 획득에 반복적으로 높은 편차. 대부분의 남성 조합은 문화-교육에서 높고, 소통은 낮은 편차를 보인다.

### 3. 개선·확장 우선순위
정보 획득 편차 높은 여성 | 30-34세 확장, 소통 낮은 그룹(남성 | 50-54세 등) 우선 개선, 문화-교육 높은 남성 | 40-44세 타겟 특화 필요.
---
"""
    return prompt.strip()
...
요청:
1. 먼저 [시각화 해석]에서 error bar 길이와 중심값의 차이, 신뢰구간이 0을 포함하는지, 명확하게 눈에 띄는 조합을 200자 이내로 설명.
2. 이어서 실질적 편차·불확실성·개입 우선순위를 예시처럼 작성.

[시각화 해석 예시]
---
[시각화 해석]
여성 | 30-34세와 남성 | 50-54세는 각각 플러스·마이너스 방향으로 error bar가 0선을 명확히 벗어난다. 여성 | 40-44세는 신뢰구간이 0을 포함해 불확실성이 크다.

### 1. 실질적 편차
여성 | 30-34세(편차 7.2, 표준오차 1.1)는 실질적으로 높다. 남성 | 50-54세(편차 -6.5, 표준오차 2.3)는 낮음.

### 2. 불확실성과 안정성
여성 | 40-44세(편차 4.2, 표준오차 3.0)는 편차는 있으나 불확실. 여성 | 30-34세는 편차 크고 오차도 작아 신뢰성 높다.
...
요청:
1. 먼저 [시각화 해석]에서 가장 높은/낮은 조합, 점수 분포 폭, 순위가 급변하는 부분 등 핵심 특징을 200자 이내로 요약.
2. 이어서 점수 분포, outlier, 일관된 패턴은 아래 예시처럼 작성.

[시각화 해석 예시]
---
[시각화 해석]
{midcat} 점수는 여성 | 30-34세, 여성 | 35-39세가 높고, 남성 | 50-54세가 낮다. 남성 | 40-44세만 예외적으로 매우 높은 수치를 보인다.

### 1. 점수 분포
여성 | 30-34세, 여성 | 35-39세는 {midcat} 점수가 일관되게 높다. 남성 | 50-54세는 낮은 편이다.
