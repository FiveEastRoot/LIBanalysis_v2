...
요청:
1. [시각화 해석]에서 히트맵 상의 뚜렷한 군집, 고점/저점의 분포, 특정 조합이 반복적으로 튀는지 200자 내외로 요약.
2. 모든 중분류에 대해 각 중분류를 기준으로 어떤 중분류의 전체 평균이 높고 낮은지 표시하고. 각 중분류에 대해서 가장 높은 조합과 낮은 조합 작성
3. 전체 경향 및 예외(조합명 명시 제외)는 기존처럼 유지.
4. 요약 문단과 조합명 포함 행동 권장점 3개 제안.
-은 모두 -로, 숫자 한 자리 소수, 조합명 반복, 소제목 포함, 전체 길이 1200자 내외.
...
### 2. 전체 경향 및 예외
* 전반적으로 정보 획득이 강하고 소통이 약한 흐름이다.
* 공간 및 이용 편의성 최고 : 여성 | 35~40(90.8) 최저 남성 | 20~25 (51.2)
